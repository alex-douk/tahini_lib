...
pub fn service(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    ServiceGenerator {
        service_ident: ident,
        _client_stub_ident: &format_ident!("{}Stub", ident),
...
impl<'a> ServiceGenerator<'a> {
    ...
    fn impl_serve_for_server(&self) -> TokenStream2 {
        ...
        let response_binding = method_idents.iter().zip(arg_pats).enumerate().map(|(index, (method_ident, arg_pats))|{
            ...
            if is_fromable {
                quote! { ::tahini_tarpc::traits::Fromable::new(#service_ident::#method_ident(self.service, ctx, #(#arg_pats),*).await)}
            } else {
        ...
        quote! {
            impl<S> ::tahini_tarpc::server::TahiniServe for #server_ident<S>
    ...
    fn enum_request(&self) -> TokenStream2 {
        ...
        quote! {
            ...
            #[allow(missing_docs)]
            //FIXME: Tahini_tarpc should maybe re-export derive macros?
    ...
    fn enum_response(&self) -> TokenStream2 {
        ...
        let variants = method_attrs
            ...
            .map(|(attrs, ty)| {
                ...
                if is_fromable {
                    quote! {::tahini_tarpc::traits::Fromable<#ty>}
                } else {
                    quote! {#ty}
                }
            })
    ...
    fn struct_client(&self) -> TokenStream2 {
        ...
        quote! {
            ...
            #vis struct #client_ident(::tahini_tarpc::client::TahiniChannel<#request_ident, #response_ident>, bool);
        }
    }
        ...
        quote! {
            impl #client_ident {
                    ...
                    ::tarpc::Response<#response_ident>>
                {
                    ...
                    ::tahini_tarpc::client::TahiniNewClient {
                        client: #client_ident(new_client.client, false),
...
fn impl_rpc_method(
    ...
    let local_args = args
        ...
        .collect::<Vec<_>>();
    let generics = input_arg_types
        .iter()
        .zip(local_args.clone())
        .map(|(ty, gen_ident)| {
            quote! { #gen_ident: ::tahini_tarpc::traits::TahiniTransformInto<#ty> + 'static + Send}
        })
        .collect::<Vec<_>>();
    ...
    if is_transformable {
        quote! {
            ...
            -> impl ::core::future::Future<
                Output = ::core::result::Result<::tahini_tarpc::traits::Fromable<#return_type>, ::tarpc::client::RpcError>
                > + '_ {
